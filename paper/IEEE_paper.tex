...
\IEEEauthorblockA{Computer and Software Engineering\\
...
Transformer-based language models have emerged as a leading paradigm for text generation, with GPT-2 \cite{radford2019language} and its subsequent variants demonstrating strong performance across a range of narrative and open-ended language tasks. Their effectiveness is primarily attributed to the self-attention mechanism \cite{vaswani2017attention}, which enables the modeling of long-range contextual dependencies that are difficult to capture using earlier neural architectures. More advanced GPT-3–style models \cite{brown2020language} further extend these capabilities through few-shot learning; however, prior studies have shown that this increased flexibility may also intensify the propagation of biases inherent in large-scale training corpora \cite{sheng2020towards}.

\subsection{Content Safety and Toxicity Detection}
To mitigate the generation of harmful content, toxicity detection frameworks such as Detoxify \cite{hanu2020detoxify} have been widely adopted, providing robust classification of abusive language, including hate speech, threats, and identity-based attacks. In addition, benchmark datasets such as RealToxicityPrompts \cite{gehman2020realtoxicityprompts} have been introduced to systematically evaluate toxic degeneration in neural language models. Motivated by these findings, the proposed system integrates machine-learning-based detection methods with complementary rule-based filtering strategies, with the objective of improving coverage while maintaining precise control over recurrent toxicity patterns.
...
\]
\[
= \sum P_{\text{baseline}} \cdot \log \frac{P_{\text{baseline}}}{P_{\text{masked}}}
\]
